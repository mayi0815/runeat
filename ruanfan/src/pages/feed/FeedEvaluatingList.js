...
export default class FeedEvaluatingList extends PureComponent {
  ...
  _renderRow = feed => (
    <EvaluatingItem onPress={this._onPressCell} feed={feed} />
  );

  _renderFooter = () => (
    <LoadMoreFooter isNoMore={this.evaluatingListStore.isNoMore} />
  );
  ...
  render() {
    ...
    return (
      <View style={styles.listView}>
        {!isFetching && (
          <FlatList
            data={this.state.dataSource.cloneWithRows(feedList.slice(0))}
